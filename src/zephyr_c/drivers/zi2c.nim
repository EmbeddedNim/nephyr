...
proc z_impl_i2c_configure*(dev: ptr device; dev_config: uint32): cint {.inline.} =
  let api: ptr i2c_driver_api = cast[ptr i2c_driver_api](dev.api)
...
proc i2c_recover_bus*(dev: ptr device): cint {.syscall.}
proc z_impl_i2c_recover_bus*(dev: ptr device): cint {.inline.} =
  let api: ptr i2c_driver_api = cast[ptr i2c_driver_api](dev.api)
  if api.recover_bus == nil:
    return -ENOSYS
...
proc i2c_slave_register*(dev: ptr device; cfg: ptr i2c_slave_config): cint {.inline.} =
  let api: ptr i2c_driver_api = cast[ptr i2c_driver_api](dev.api)
  if api.slave_register == nil:
    return -ENOSYS
...
proc i2c_slave_unregister*(dev: ptr device; cfg: ptr i2c_slave_config): cint {.inline.} =
  let api: ptr i2c_driver_api = cast[ptr i2c_driver_api](dev.api)
  if api.slave_unregister == nil:
    return -ENOSYS
...
proc z_impl_i2c_slave_driver_register*(dev: ptr device): cint {.inline.} =
  let api: ptr i2c_slave_driver_api = cast[ptr i2c_slave_driver_api](dev.api)
...
proc z_impl_i2c_slave_driver_unregister*(dev: ptr device): cint {.inline.} =
  let api: ptr i2c_slave_driver_api = cast[ptr i2c_slave_driver_api](dev.api)
...
proc i2c_write*(dev: ptr device; buf: ptr uint8; num_bytes: uint32; `addr`: uint16): cint {.
    inline.} =
  var msg: i2c_msg
  msg.buf = cast[ptr uint8](buf)
  msg.len = num_bytes
  msg.flags = I2C_MSG_WRITE or I2C_MSG_STOP
  return i2c_transfer(dev, addr(msg), 1, `addr`)
...
proc i2c_read*(dev: ptr device; buf: ptr uint8; num_bytes: uint32; `addr`: uint16): cint {.
  ...
  msg.buf = buf
  msg.len = num_bytes
  msg.flags = I2C_MSG_READ or I2C_MSG_STOP
  return i2c_transfer(dev, addr(msg), 1, `addr`)
...
proc i2c_write_read*(dev: ptr device; `addr`: uint16; write_buf: pointer;
    ...
    inline.} =
  var msg: array[2, i2c_msg]
  msg[0].buf = cast[ptr uint8](write_buf)
  msg[0].len = num_write
  msg[0].flags = I2C_MSG_WRITE
  msg[1].buf = cast[ptr uint8](read_buf)
  msg[1].len = num_read
  msg[1].flags = I2C_MSG_RESTART or I2C_MSG_READ or I2C_MSG_STOP
...
proc i2c_burst_read*(dev: ptr device; dev_addr: uint16; start_addr: uint8;
                    buf: ptr uint8; num_bytes: uint32): cint {.inline.} =
  return i2c_write_read(dev, dev_addr, addr(start_addr), sizeof((start_addr)), buf,
                       num_bytes)
...
proc i2c_burst_write*(dev: ptr device; dev_addr: uint16; start_addr: uint8;
                     buf: ptr uint8; num_bytes: uint32): cint {.inline.} =
  var msg: array[2, i2c_msg]
  msg[0].buf = addr(start_addr)
  msg[0].len = 1'u
  msg[0].flags = I2C_MSG_WRITE
  msg[1].buf = cast[ptr uint8](buf)
  msg[1].len = num_bytes
  msg[1].flags = I2C_MSG_WRITE or I2C_MSG_STOP
...
proc i2c_reg_read_byte*(dev: ptr device; dev_addr: uint16; reg_addr: uint8;
                       value: ptr uint8): cint {.inline.} =
  return i2c_write_read(dev, dev_addr, addr(reg_addr), sizeof((reg_addr)), value,
                       sizeof((value[])))
...
proc i2c_reg_write_byte*(dev: ptr device; dev_addr: uint16; reg_addr: uint8;
                        value: uint8): cint {.inline.} =
  var tx_buf: array[2, uint8] = [reg_addr, value]
...
proc i2c_reg_update_byte*(dev: ptr device; dev_addr: uint8; reg_addr: uint8;
                         mask: uint8; value: uint8): cint {.inline.} =
  var
    old_value: uint8
    new_value: uint8
  var rc: cint
  rc = i2c_reg_read_byte(dev, dev_addr, reg_addr, addr(old_value))
  if rc != 0:
    return rc
  new_value = (old_value and not mask) or (value and mask)
...
var I2C_DECLARE_CLIENT_CONFIG* {.importc: "I2C_DECLARE_CLIENT_CONFIG",
                               header: "i2c.h".}: int
proc I2C_GET_MASTER*(_conf: untyped) {.importc: "I2C_GET_MASTER", header: "i2c.h".}
proc I2C_GET_ADDR*(_conf: untyped) {.importc: "I2C_GET_ADDR", header: "i2c.h".}
## *
##  @}
##
