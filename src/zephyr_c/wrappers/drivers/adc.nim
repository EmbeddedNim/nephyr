...
type
  adc_channel_cfg* {.importc: "adc_channel_cfg", header: "adc.h", bycopy.} = object
    ...
    when defined(CONFIG_ADC_CONFIGURABLE_INPUTS):
      ...
      ##
      var input_positive* {.header: "adc.h".}: uint8
      ## *
      ##  Negative ADC input (used only for differential channels).
      ##  This is a driver dependent value that identifies an ADC input to be
      ##  associated with the channel.
      ##
      var input_negative* {.header: "adc.h".}: uint8
...
discard "forward decl of adc_sequence"
type
  adc_action* {.size: sizeof(cint).} = enum ## * The sequence should be continued normally.
...
proc z_impl_adc_channel_setup*(dev: ptr device; channel_cfg: ptr adc_channel_cfg): cint {.
    inline, importc: "z_impl_adc_channel_setup".} =
  let api: ptr adc_driver_api
...
proc z_impl_adc_read*(dev: ptr device; sequence: ptr adc_sequence): cint {.inline,
    importc: "z_impl_adc_read".} =
  let api: ptr adc_driver_api
...
proc adc_ref_internal*(dev: ptr device): uint16 {.inline, importc: "adc_ref_internal".} =
  let api: ptr adc_driver_api
